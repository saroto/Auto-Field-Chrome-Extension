...
function makeFieldHeader(field) {
...
function makeOptionGroup(field, inputType) {
    ...
    (field.options ?? []).forEach((opt) => {
        ...
        if (inputType === "radio")
            control.name = `popup_${field.name}`;
...
export function renderFields(fields, container) {
    ...
    if (fields.length === 0) {
        container.appendChild(makeEmptyState("No fields here. Rescan once the form has loaded."));
        return;
    }
