...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadFields() {
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0] && tabs[0].id) {
                chrome.tabs.sendMessage(tabs[0].id, { action: "GET_FIELDS" }, (response) => {
                    ...
                    const keysToLoad = currentFields.map((f) => `autofill_${activeProfile}_${f.name}`);
                    chrome.storage.sync.get(keysToLoad, (result) => {
                        currentFields.forEach((field) => {
                            ...
                            if (inputEl &&
                                ...
                                inputEl.value = result[`autofill_${activeProfile}_${field.name}`];
                            }
                        });
                    });
                });
    ...
    fillAllBtn.addEventListener("click", () => {
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0] && tabs[0].id) {
                chrome.tabs.sendMessage(tabs[0].id, { action: "FILL_ALL_FIELDS", profile: activeProfile }, (response) => {
    ...
    magicFillBtn.addEventListener("click", () => {
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs[0] && tabs[0].id) {
                chrome.tabs.sendMessage(tabs[0].id, { action: "MAGIC_FILL" }, (response) => {
                    if (chrome.runtime.lastError) {
