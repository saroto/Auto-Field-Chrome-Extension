...
function resolveValue(
  ...
  if (type === "tel" || nameAttr.includes("phone") || nameAttr.includes("tel")
    ...
    || nameAttr.includes("cellphone")) return pick(PHONE_NUMBERS);
  ...
  // ── Age ───────────────────────────────────────────────────────────────
  if (nameAttr.includes("age")) return `${20 + Math.floor(Math.random() * 40)}`;
  ...
  if (nameAttr.includes("username") || nameAttr.includes("user_name")
    || nameAttr.includes("login") || nameAttr.includes("userid")) return pick(USERNAMES);

  // ── Website / URL ─────────────────────────────────────────────────────
  if (nameAttr.includes("website") || nameAttr.includes("url")
    || nameAttr.includes("homepage") || nameAttr.includes("site")
    || type === "url") return pick(WEBSITES);
  ...
  if (nameAttr.includes("company") || nameAttr.includes("org")
    || nameAttr.includes("employer") || nameAttr.includes("workplace")) return pick(COMPANY_NAME_EN);

  // ── Job / Title / Position ────────────────────────────────────────────
  if (nameAttr.includes("job") || nameAttr.includes("title")
    || nameAttr.includes("position") || nameAttr.includes("role")
    || nameAttr.includes("occupation") || nameAttr.includes("profession")) return pick(JOB_TITLES);

  // ── Name fields ───────────────────────────────────────────────────────
  if (nameAttr.includes("middle")) return pick(["A", "B", "J", "M", "R", "K", "L"]);
  if (nameAttr.includes("first") || nameAttr.includes("fname") || nameAttr.includes("f_name")
    || nameAttr.includes("given")) return pick(FIRST_NAME_EN);
  if (nameAttr.includes("last") || nameAttr.includes("lname") || nameAttr.includes("l_name")
    || nameAttr.includes("surname") || nameAttr.includes("family")) return pick(LAST_NAME_EN);
  ...
  if (nameAttr.includes("address") || nameAttr.includes("street")
    || nameAttr.includes("addr")) return pick(STREET_ADDRESSES);
  if (nameAttr.includes("city") || nameAttr.includes("town")
    || nameAttr.includes("locality")) return pick(CITIES);
  if (nameAttr.includes("state") || nameAttr.includes("province")
    || nameAttr.includes("region")) return pick(STATES_PROVINCES);
  if (nameAttr.includes("country") || nameAttr.includes("nation")) return pick(COUNTRIES);
  if (nameAttr.includes("zip") || nameAttr.includes("postal")
    || nameAttr.includes("postcode")) return `${Math.floor(10000 + Math.random() * 90000)}`;
  ...
  if (el.tagName === "TEXTAREA" || nameAttr.includes("desc")
    ...
    || nameAttr.includes("remark") || nameAttr.includes("instruction")) return pick(LOREM_PARAGRAPHS);
  ...
  if (nameAttr.includes("qty") || nameAttr.includes("quantity")
    || nameAttr.includes("count")) return `${1 + Math.floor(Math.random() * 50)}`;
