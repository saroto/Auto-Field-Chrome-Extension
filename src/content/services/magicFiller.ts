...
function getRandomSelectValue(select: HTMLSelectElement): string {
  ...
  const validOptions = options.filter(
    (o) => o.value && o.value.toLowerCase() !== "select" && o.textContent?.toLowerCase() !== "select",
  );
  ...
  allInputs.forEach((el) => {
    ...
    if (!IGNORED_INPUT_TYPES.includes(type as any)) {
      ...
      if (nameAttr.includes("email") || type === "email") {
        ...
        (input as HTMLInputElement).style.backgroundColor = MAGIC_FILL_BG_COLOR;
        setTimeout(() => {
        ...
        // Visual feedback
        const originalCheckboxBg = (input as HTMLInputElement).style.backgroundColor;
        (input as HTMLInputElement).style.backgroundColor = MAGIC_FILL_BG_COLOR;
        setTimeout(() => {
          (input as HTMLInputElement).style.backgroundColor = originalCheckboxBg;
        }, 800);
        return; // Skip the generic value setting below
      } else if (type === "select") {
