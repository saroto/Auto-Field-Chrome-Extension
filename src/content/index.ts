...
/**
 * The popup re-injects this script into tabs that predate the extension being
 * loaded or reloaded, and it can only tell per-tab — not per-frame — whether a
 * listener already exists. This flag makes a second injection a no-op so we
 * never end up with two listeners answering the same message.
 */
interface AutoFillWindow extends Window {
  ...
  chrome.runtime.onMessage.addListener(
    (request: ContentMessage, _sender, sendResponse) => {
      if (request.action === "GET_FIELDS") {
        // Resolve the dialog once and reuse it, so the popup's badge can't
        // disagree with the set of fields we actually collected.
        const modal = fieldDetector.findActiveModal();
        const allInputs = fieldDetector.getAllInputs(modal ?? document);
        const fields: Field[] = [];
        ...
        sendResponse({ fields, inDialog: modal !== null });
...
const autoFillWindow = window as AutoFillWindow;
if (!autoFillWindow.__autoFillContentLoaded) {
  autoFillWindow.__autoFillContentLoaded = true;
  activate();
}
